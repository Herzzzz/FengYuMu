...
$cases = @(
    [pscustomobject]@{
        Name='技能列表'; File='skill-tab-triple-throw.png'; MinimumHits=18
        Scenes=@('技能:'); Expected=@('冒险岛勇士@','暗器伤人@','假动作@','武器用毒液@')
    },
    [pscustomobject]@{
        Name='装备详情 25/0 纠错'; File='equipment-rat-mouth.png'; MinimumHits=15
        Scenes=@('装备物品:'); Expected=@('老鼠嘴@','需要等级：25@','需要力量：0@','需要敏捷：0@','需要智力：0@','需要运气：0@','智力：+14@')
    },
    [pscustomobject]@{
        Name='装备详情 50/0 纠错'; File='equipment-crimsonheart-cloak.png'; MinimumHits=17
        Scenes=@('装备物品:'); Expected=@('需要等级：50@','需要力量：0@','需要敏捷：0@','需要智力：0@','需要运气：0@','魔法防御力：38@')
    },
    [pscustomobject]@{
        Name='低分辨率人物属性'; File='character-stats-warrior.png'; MinimumHits=27
        Scenes=@('人物:','人物视觉:'); Expected=@('名称@','职业@','生命值@','能力值点数@','物理防御力@','暴击伤害@','跳跃力@')
    },
    [pscustomobject]@{
        Name='人物属性与装备并存'; File='character-equipment-dark-knight.png'; MinimumHits=40
        Scenes=@('人物:','装备物品:','人物视觉:'); Expected=@('角色属性@','类型：枪@','物理攻击力：66@','名称@','能力值点数@','魔法防御力@','跳跃力@')
    },
    [pscustomobject]@{
        Name='任务长句'; File='quest-dr-kim.png'; MinimumHits=11
        Scenes=@('任务:'); Expected=@('任务助手@','哦……这是我亲手打造的旷世杰作！有了这张蓝图，我就能造出最坚不可摧的机器人！@')
    },
    [pscustomobject]@{
        Name='NPC 对话'; File='npc-jack-dialogue.png'; MinimumHits=5
...
foreach ($mode in @('maximum','balanced','minimum')) {
    foreach ($case in $cases) {
        ...
        if ($hits -lt $case.MinimumHits) { throw "$($case.Name)/$mode 命中不足：$hits < $($case.MinimumHits)" }
...
Write-Output 'v3.0 公开实机回归：3 档 × 7 个高价值场景全部通过（低价值背包图未计分）'
